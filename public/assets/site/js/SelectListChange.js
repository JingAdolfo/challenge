...
function selectListNewRoomSpaceChange(element, url, param, defaultOptionName,counter,course_counter) 
{

    form = $("#course"+course_counter).find('form');
    var elementName = "#" + element;
    ...
    $(form).find(elementName + "> option").remove();
    $(form).find(elementName).append($("<option />").val("").text("---" + defaultOptionName + "---"));

    $(form).find("#modal_course_id_"+counter).find('option').each(function(){
        $(this).removeClass('hidden');
    });

    var count_course_capacity = 1;
    var course_capacity_id = param.course_capacity_id;
    $(form).find("#modal_course_id_"+counter+" option").each(function(){
     ...
     $.ajax({
        ...
        success: function (data) 
        {
                $(form).find(elementName).empty();
                $(form).find(elementName).append('<option value="0"></option>');
                ...
                {  
                    ...
                    // {
                        ...
                        $(form).find(elementName).append($("<option/>").val(item.value).text(item.text));
                        count_arr++;
                    // }
                ...
                {      
                        $(form).find("#modal_teacher_name_"+counter).append('<option data-course_capacity_id="'+item.counter+'" value="'+item.teacher_id+'">'+item.teacher_name+'</option>');
                ...
                if(data_counter != 0)
                {
                    item = get_random(option_arr);
                        ...
                        value1 = item[1];
                        value2 = item[2];
                        $(form).find("#modal_room_id_"+counter).select2().select2('val',value);
                    if(item[1] != null)
                    {
                        // $("#modal_teacher_name_"+counter).val(value2);
                        // $(form).find("#modal_teacher_name_"+counter).append('<option value="'+value1+'">'+value2+'</option>');
                        $(form).find("#modal_teacher_name_"+counter).select2().select2('val',value1);
                        $(form).find("#modal_teacher_name_id_"+counter).val(value1);
                        $(form).find("#modal_teacher_name_"+counter).removeAttr('data-teacher_id');
                        $(form).find("#modal_teacher_name_"+counter).attr('data-teacher_id',value1);
                    }
                }
    ...
    $(form).find(elementName).trigger("change");
}

function selectListAddNewRoomSpaceChange(element, url, param, defaultOptionName,counter,form) 
{
...
function selectListRoomSpaceChange(element, url, param, defaultOptionName,counter,course_counter) 
{
    ...
    $("#modal_course_id_"+counter).find('option').each(function(){
     ...
     $.ajax({
        ...
        success: function (data) 
        {
                ...
                // $.map(data[0], function (item) 
                // {      
                //         $("#course"+course_counter).find("#modal_teacher_name_"+counter).append('<option data-course_capacity_id="'+item.counter+'" value="'+item.teacher_id+'">'+item.teacher_name+'</option>');
...
function selectListAddRoomSpaceChange(element, url, param, defaultOptionName,counter,form) 
{
        ...
        success: function (data) 
        {
                ...
                if(data_counter != 0)
                {
                    item = get_random(option_arr);
                        ...
                        value1 = item[1];
                        value2 = item[2];
                        $(form).find("#modal_room_id_"+counter+" [value='"+value+"']").prop("selectListNewRoomSpaceChange",true);
                    if(item[1] != null)
                    {
                        // $("#modal_teacher_name_"+counter).val(value2);
                        $(form).find("#modal_teacher_name_"+counter).append('<option value="'+value1+'">'+value2+'</option>');
                        $(form).find("#modal_teacher_name_"+counter+" [value='"+value1+"']").prop("selected",true);
                        $(form).find("#modal_teacher_name_id_"+counter).val(value1);
                        $(form).find("#modal_teacher_name_"+counter).removeAttr('data-teacher_id');
                        $(form).find("#modal_teacher_name_"+counter).attr('data-teacher_id',value1);
                    }
...
function selectListNewChangeClass(element, url, param, element2,defaultOptionName,date,date_end,course_counter) 
{
            ...
            $("#course"+course_counter).find(elementName).each(function(){
                ...
                {      
                    if(item.count == count)
                    {
                        ...
                        {
                             $.ajax({
                                url: "../scheduler/courseCapacityNewRoomTeacherDataJson",
                                data: 
                                {
                                    'course_capacity_id' : 8, 
                                    'class_id' : class_id,
